...
// MONGODB CONNECTION
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.6reqm.mongodb.net/${process.env.DB_NAME}?retryWrites=true&w=majority`;
// mongodb+srv://mydbuser1:<password>@cluster0.6reqm.mongodb.net/myFirstDatabase?retryWrites=true&w=majority
...
client.connect((err) => {
  //================================== ALL COLLECTION ======================================
  const apartmentCollection = client.db('apartmentSharkRedux').collection('apartments');
  const registrationCollection = client.db('apartmentSharkRedux').collection('registration');
  const adminCollection = client.db('apartmentSharkRedux').collection('admin');
...
// Root:
app.get('/', (req, res) => {
  res.send('The Apartment Shark Server is running');
});
